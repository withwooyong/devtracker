...
### 남겨둔 후속 (2026-04-24 기준)

- **다크모드 토글 UI** (`next-themes` 도입 + 헤더 토글) (MEDIUM) — 토큰/variant 준비 완료, 실제 UI만 남음
- **AlertDialog 도입으로 `window.confirm()` 교체** (MEDIUM) — 스프린트 삭제 등에서 여전히 사용 중
- **success Button variant** 추가해 "스프린트 완료" 버튼의 하드코딩 `bg-emerald-600` 제거 (LOW) — 현재 라이트/다크 대응 안 됨
- **`components/layout/*`·`components/common/*` 토큰 치환** (LOW) — header, sidebar, project-tabs, main-layout, notification-dropdown, attachment-list, user-avatar 등. 영향도 낮아 보류 가능
- **lucide-react 1.8.0 버전 출처 재확인** (LOW)