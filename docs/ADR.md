...
**청크 2 — 남은 페이지 전환 완료** (`79a3b50` + `010f491` + `b9d8368`):
- 2-a: 대시보드 · 프로젝트 목록 · 프로젝트 메인(이슈 목록 + 필터). Popover(@radix-ui/react-popover), Table 컴포넌트 추가. 필터 저장 드롭다운 커스텀 구현 → shadcn Popover로 교체하면서 **B안 묶음 2(외부 클릭 닫힘) 자연 해소**. `Button asChild + Link`로 CTA 통일. filter create/delete mutation `onError` 토스트 + JSON.parse try/catch
- 2-b: 로그인(MainLayout-less) · 전역 설정 · 프로젝트 설정. Textarea 첫 사용처. disabled Input에 `readOnly` 병기로 React controlled-input 경고 제거. webhook secret 상태 기계 버그(제거 취소 시 state 잔류로 silent drop) 같이 수정. link variant 버튼 `focus-visible:ring-offset-2`로 ring 잘림 방지. password placeholder a11y 개선("••••••" → "비밀번호 입력")
- 2-c: 배포 목록/신규 · 스프린트 목록/신규/상세. **`SPRINT_STATUS_VARIANT`** 모듈 스코프 매핑(slate/blue/emerald) 도입 — 기존 `SPRINT_STATUS_COLORS`(하드코딩 Tailwind) 대체. Badge `rounded-full` 오버라이드로 pill 유지. BurndownChart는 그대로 유지. `<details>`는 그대로 두되 pre 배경 `bg-muted/50`로 토큰화

**청크 3 — 다크모드 + 생명주기 확장 완료** (`651f9f4` + `eb372e1` + `abc3a8a` + `5211ab0`, 2026-04-24 후반):
- 3-a 다크모드 토글: `next-themes` 0.4.6 + `ThemeProvider`(attribute="class", system default, disableTransitionOnChange) + `<html suppressHydrationWarning>`. 헤더에 `ThemeToggle`(Sun/Moon rotate+scale 전환, mounted guard로 SSR 불일치 차단). `@custom-variant dark` + `.dark` OKLCH 토큰이 자동 활성화되며 이미 토큰화된 모든 페이지가 즉시 전환
- 3-b success Button variant: cva에 `success: "bg-emerald-600 text-white ... dark:bg-emerald-500 dark:hover:bg-emerald-400"` 추가. 스프린트 완료 버튼 인라인 색 제거. **handoff 제안 `text-primary-foreground` 대신 `text-white` 채택** — slate 테마에서 primary-foreground가 다크모드에 near-black으로 뒤집혀 emerald 위 대비가 깨지는 문제
- 3-c AlertDialog: `@radix-ui/react-alert-dialog` + 공식 shadcn 래퍼. 스프린트 삭제 + 첨부파일 삭제(handoff는 1곳 예상했으나 grep으로 2곳 확인) 모두 교체. `AlertDialogAction`에 `disabled={mutation.isPending}`로 애니메이션 중 중복 DELETE 차단. 더블 `buttonVariants()` 적용은 `cn()`의 `twMerge`가 자동 해소
- 3-d 레이아웃/공통 토큰 치환: header/project-tabs/main-layout/notification-dropdown/attachment-list 5개 파일. `bg-white→bg-background/card/popover`, `border-gray-*→border-border`, `text-gray-*→text-muted-foreground/foreground`, `text-blue-600→text-primary`. **sidebar.tsx와 user-avatar.tsx는 스킵** — 전자는 "항상 다크" 디자인 의도, 후자는 사용자 식별 색 팔레트. 알림 읽음 전 행은 `bg-accent/50`이 다크에서 hover 상태와 구분 불가 → `bg-blue-50/60 dark:bg-blue-950/30`로 파랑 시맨틱 복원

### 남겨둔 후속 (2026-04-24 청크 3 후 기준)

- **다크모드 실기 검증** — 토큰 체계 전환은 완료됐으나 각 페이지(특히 알림 드롭다운, 첨부 리스트, 스프린트, 이슈 상세)의 라이트/다크 왕복 가시 검증은 미수행. production 배포 후 또는 `pnpm dev` 실기 확인 필요
- **lucide-react 1.8.0 버전 출처 재확인** (LOW) — 이월
- **BurndownChart 와이드 데스크톱(2000px+) 캡** (LOW) — 이월, 현재 레이아웃에서 무영향